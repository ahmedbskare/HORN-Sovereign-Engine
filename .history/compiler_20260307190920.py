...
for i in range(100):  # (يمكن زيادتها لملء الملف)
    horn_db.insert_secure_record(
        "Logs", {
        "ts": str(
            datetime.now()), "event": "INIT", "node": i})
  ...
  """
   ...
   def __init__(self):
    ...
    def generate_quantum_keys(self):
        ...
        matrix_a = [[Random.getrandbits(16) for _ in range(
            self.lattice_dimension)] for _ in range(self.lattice_dimension)]
        secret_s = [Random.getrandbits(1)
    ...
    def encrypt_quantum_stream(self, data_stream):
        ...
        noise = [Random.gauss(0, self.noise_parameter)
